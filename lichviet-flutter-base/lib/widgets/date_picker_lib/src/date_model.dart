import 'package:lichviet_flutter_base/widgets/date_picker_lib/src/date_format.dart';
...
class CommonPickerModel extends BasePickerModel {
  ...
  CommonPickerModel({LocaleType? locale})
      : this.locale = locale ?? LocaleType.vi;
...
class DatePickerModel extends CommonPickerModel {
  ...
  DatePickerModel(
      ...
      bool? isChangeDay})
      : super(locale: locale) {
    this.maxTime = maxTime ?? DateTime(2040, 1, 1);
    ...
    if (this.currentTime.month == 12) {
      _currentMiddleIndex =
          this.currentTime.month - (isChangeDay == true ? minMonth : 0);
    } else {
  ...
  void fillMiddleLists() {
    ...
    if (month == null) {
      ...
      middleList = [];
      // middleList.add('Tháng 1');
      for (var element in month!) {
        middleList.add(_localeMonth(element));
      }
    }
  }
  ...
  void fillRightLists() {
    ...
    if (day == null) {
      ...
      for (var element in day!) {
        rightList.add('$element ${_localeDay()}');
      }
    }
  }
  ...
  void setMiddleIndex(int index) {
    ...
    int minDay = _minDayOfCurrentMonth();
    if (leapMonth != null) {
      if (_currentRightIndex > leapMonth!) {
