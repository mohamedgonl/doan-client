...
class BuchheimWalkerAlgorithm extends Algorithm {
  ...
  num getSpacing(Graph graph, Node? leftNode, Node rightNode) {
    ...
    if (leftNode != null && leftNode.pids!.isNotEmpty) {
      // int indexWife = graph.nodes.indexWhere(
      //     (element) => (leftNode.pid!.contains(element.keyWife?.value)));

      // if (indexWife != -1) {
      //   separation += (graph.nodes[indexWife].width +
      //           configuration.getSiblingSeparation())
      //       .toInt();
      // }
      separation += (leftNode.width * leftNode.pids!.length +
              configuration.getSiblingSeparation())
          .toInt();
    }
  ...
  void shiftCoordinates(Graph graph, double shiftX, double shiftY) {
    graph.nodes.forEach((node) {
      ...
      // if (node.indexWife != null) {
      //   node.position = (Offset(graph.nodes[node.indexWife!].x + 250,
      //       graph.nodes[node.indexWife!].y + shiftY));
      // } else {
      //   node.position = (Offset(node.x + shiftX, node.y + shiftY));
      // }

      // // tesst
      // if (node.key?.value.toString() == "2") {
      //   node.position = (Offset(
      //       (graph.nodes[0].x + graph.nodes[1].x) / 2, node.y + shiftY));
      // }
    });
  }

  void setPositionNodeWife(Graph graph) {
    graph.nodes.forEach((node) {
      //print(node.width);
      List<String> pidsChuaXoa = [];
      for (var element in (node.pids ?? [])) {
        if (!element.contains('da_xoa')) {
          pidsChuaXoa.add(element);
        }
      }
      for (int i = 0; i < (pidsChuaXoa).length; i++) {
        int indexWife = graph.nodes
            .indexWhere((element) => element.memberId == pidsChuaXoa[i]);
        if (indexWife != -1) {
          //print(graph.nodes[indexWife].width);
          graph.nodes[indexWife].position = Offset(
              node.x +
                  (node.width + configuration.siblingSeparation) * (i + 1) -
                  (i > 0 ? 38.w * (i * 2) - 6 : 0),
              node.y);
        }
      }
