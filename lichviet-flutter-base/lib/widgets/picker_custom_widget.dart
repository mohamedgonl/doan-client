          ...
          Positioned.fill(
            child: InkWell(
              ...
              onTapUp: (details) {
                ...
                final midPoint = referenceBox.size.height / 2;
                final sizeTapBox =
                    referenceBox.globalToLocal(details.globalPosition);
                if (sizeTapBox.dy > midPoint - (widget.itemExtent / 2) &&
                    sizeTapBox.dy < midPoint + (widget.itemExtent / 2)) {
                  widget.onTapToSelect();
                }
                // tap to bottom
                if (sizeTapBox.dy > midPoint + (widget.itemExtent / 2) &&
                    sizeTapBox.dy <
                        midPoint +
                            (widget.itemExtent / 2) +
                            widget.itemExtent / widget.squeeze) {
                    ...
                    sizeTapBox.dy <
                        midPoint +
                            ...
                            widget.itemExtent /
                                ...
                                widget.squeeze) {
                  widget.scrollController!.animateToItem(
                      widget.scrollController!.selectedItem + 2,
                      duration: const Duration(milliseconds: 200),
                      curve: Curves.linear);
                }
                if (sizeTapBox.dy >
                    midPoint +
                        (widget.itemExtent / 2) +
                        widget.itemExtent / widget.squeeze +
                        widget.itemExtent / widget.squeeze / widget.squeeze) {
                  widget.scrollController!.animateToItem(
                      widget.scrollController!.selectedItem + 3,
                      duration: const Duration(milliseconds: 200),
                      curve: Curves.linear);
                }
                // onTap to top
                if (widget.scrollController!.selectedItem != 0) {
                  if (sizeTapBox.dy < midPoint - (widget.itemExtent / 2) &&
                      sizeTapBox.dy >
                          midPoint -
                              (widget.itemExtent / 2) -
