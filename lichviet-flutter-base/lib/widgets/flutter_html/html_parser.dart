...
import 'package:lichviet_flutter_base/widgets/flutter_html/image_render.dart';
import 'package:lichviet_flutter_base/widgets/flutter_html/src/anchor.dart';
import 'package:lichviet_flutter_base/widgets/flutter_html/src/css_parser.dart';
import 'package:lichviet_flutter_base/widgets/flutter_html/src/html_elements.dart';
import 'package:lichviet_flutter_base/widgets/flutter_html/src/layout_element.dart';
import 'package:lichviet_flutter_base/widgets/flutter_html/src/navigation_delegate.dart';
import 'package:lichviet_flutter_base/widgets/flutter_html/src/utils.dart';
import 'package:lichviet_flutter_base/widgets/flutter_html/style.dart';
...
class HtmlParser extends StatelessWidget {
  ...
  static List<String> getImgUrlsToCacheFromHtmlStr(String htmlStr) {
    final customRenderTags = [
      "imgfullwidth",
      "imgfixedsize",
      "imgoriginalsize",
    ];
    final tagList = Html.tags;
    // List<String>.from(REPLACED_ELEMENTS);
    tagList.addAll(customRenderTags);

    final domTree = HtmlParser.genDomTreeForCacheResource(
        htmlStr, customRenderTags, tagList);
    List<String> listUrl = [];
    _loopChildGetUrlImg(listUrl, domTree);
    return listUrl;
  }

  static _loopChildGetUrlImg(
      List<String> listUrl, StyledElement styledElement) {
    final imgUrlFromElement = styledElement.attributes['src'] ?? "";
    if (styledElement.name == 'imgfullwidth') {
      // append img
      var widthSv = double.tryParse(styledElement.attributes['width'] ?? "") ??
          ScreenUtil().screenWidth;
      var heightSv =
          double.tryParse(styledElement.attributes['height'] ?? "0") ?? 0;
      if (heightSv != 0 && imgUrlFromElement.isNotEmpty) {
        final scale = WidgetsBinding.instance.window.devicePixelRatio;
        final imageUrl = ImageNetworkUtils.getNetworkResizeUrl(
          ImageNetworkUtils.getNetworkUrl(url: imgUrlFromElement),
          ScreenUtil().screenWidth,
          ScreenUtil().pixelRatio ?? scale,
          height: heightSv * (ScreenUtil().screenWidth / widthSv),
        );
        listUrl.add(imageUrl);
      }
    } else if (styledElement.name == 'imgfixedsize') {
      // append img
      var widthSv = double.tryParse(styledElement.attributes['width'] ?? "") ??
          ScreenUtil().screenWidth;
      var heightSv =
    ...
    } else if (styledElement.name == 'imgoriginalsize') {
      listUrl.add(imgUrlFromElement);
    }
    for (var childElement in styledElement.children) {
      _loopChildGetUrlImg(listUrl, childElement);
    }
  }

  static StyledElement genDomTreeForCacheResource(
    String htmlString,
    List<String> customRenderTags,
    List<String> tagsList,
  ) {
    final html = HtmlParser.parseHTML(htmlString);
