...
class QuanLyThanhVienBloc
  ...
  QuanLyThanhVienBloc(this._quanLyThanhVienDataSource, this._userUsecase)
    ...
    on<QuanLyThanhVienEvent>((event, emit) async {
      ...
      if (event is XacThucMaLichVietEvent) {
        emit(MaLichVietLoading());
        if (event.maLichViet.isNullOrEmpty) {
          emit(MaLichVietEmpty());
        } else {
          final res = await _quanLyThanhVienDataSource.xacThuMaLichViet(
              event.maLichViet, event.giaPhaId);
          final fin = res.fold(
              (l) => MaLichVietKhongHopLe((l as ServerException).error),
              (r) => MaLichVietHopLe(r));
          emit(fin);
        }
      }
